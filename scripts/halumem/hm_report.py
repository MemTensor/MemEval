"""HaluMem experiment report — delegates shared logic to report_base."""

import os
import sys

sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), ".."))

from utils.report_base import BenchmarkReport, render_latency, report_main

QUESTION_TYPES = [
    "Basic Fact Recall",
    "Memory Boundary",
    "Memory Conflict",
    "Generalization & Application",
    "Multi-hop Inference",
    "Dynamic Update",
]


class HaluMemReport(BenchmarkReport):
    benchmark_name = "HaluMem"
    results_prefix = "halumem"
    grades_suffix = "hm_grades"
    default_script = "run_halumem_eval.sh"
    config_params = (
        "WORKERS", "LLM_WORKERS", "TOPK", "VARIANT", "NUM_RUNS",
        "SAVE_MODEL_INPUT", "CLEAR", "WAIT_AFTER_INGEST",
        "ALLOW_EMPTY_SEARCH", "SKIP_FAILED_SEARCH", "SKIP_FAILED_ANSWER",
        "SKIP_FAILED_JUDGE",
    )

    def render_scores(self, lines, grades):
        metrics = grades.get("metrics", {})
        category_scores = grades.get("category_scores", {})

        lines.append("## Evaluation Results")
        lines.append("")

        lines.append("### Overall Scores")
        lines.append("")
        lines.append("| Metric | Score |")
        lines.append("|--------|-------|")

        judge_score = metrics.get("llm_judge_score", 0)
        judge_std = metrics.get("llm_judge_std", 0)
        lines.append(f"| QA Accuracy (LLM-as-Judge) | {judge_score:.4f} ± {judge_std:.4f} |")

        lexical = metrics.get("lexical", {})
        for key, label in [
            ("f1", "F1"), ("rouge1_f", "ROUGE-1"), ("rouge2_f", "ROUGE-2"),
            ("rougeL_f", "ROUGE-L"), ("bleu1", "BLEU-1"), ("bleu4", "BLEU-4"),
            ("meteor", "METEOR"),
        ]:
            if key in lexical:
                v = lexical[key]
                lines.append(f"| {label} | {v:.4f} |" if v is not None else f"| {label} | - |")

        semantic = metrics.get("semantic", {})
        for key, label in [("bert_f1", "BERTScore F1"), ("similarity", "Semantic Similarity")]:
            if key in semantic:
                v = semantic[key]
                lines.append(f"| {label} | {v:.4f} |" if v is not None else f"| {label} | - |")

        lines.append("")

        if category_scores:
            lines.append("### By Question Type (QA Accuracy)")
            lines.append("")
            lines.append("| Question Type | QA Accuracy | Questions |")
            lines.append("|---------------|-------------|-----------|")
            for qt in QUESTION_TYPES:
                if qt in category_scores:
                    cat = category_scores[qt]
                    lines.append(
                        f"| {qt} | {cat.get('llm_judge_score', 0):.4f} | "
                        f"{cat.get('total', 0)} |"
                    )
                else:
                    lines.append(f"| {qt} | — | 0 |")
            extra_cats = set(category_scores.keys()) - set(QUESTION_TYPES)
            for qt in sorted(extra_cats):
                cat = category_scores[qt]
                lines.append(
                    f"| {qt} | {cat.get('llm_judge_score', 0):.4f} | "
                    f"{cat.get('total', 0)} |"
                )
            lines.append("")

        difficulty_scores = grades.get("difficulty_scores", {})
        if difficulty_scores:
            lines.append("### By Difficulty")
            lines.append("")
            lines.append("| Difficulty | QA Accuracy | Questions |")
            lines.append("|------------|-------------|-----------|")
            for diff in sorted(difficulty_scores.keys()):
                d = difficulty_scores[diff]
                lines.append(
                    f"| {d.get('difficulty_name', diff)} | "
                    f"{d.get('llm_judge_score', 0):.4f} | "
                    f"{d.get('total', 0)} |"
                )
            lines.append("")

        pipeline_status = grades.get("pipeline_status", {})
        if pipeline_status:
            lines.append("### Pipeline Status")
            lines.append("")
            lines.append(
                "| Stage | Failed Users | Failed Questions | "
                "Skipped Records | Status Counts |"
            )
            lines.append(
                "|-------|--------------|------------------|"
                "-----------------|---------------|"
            )
            for stage in ("search", "answer", "eval"):
                data = pipeline_status.get(stage)
                if not data:
                    continue
                counts = data.get("status_counts") or {}
                count_text = ", ".join(f"{k}={v}" for k, v in sorted(counts.items())) or "-"
                lines.append(
                    f"| {stage} | {data.get('failed_users', 0)} | "
                    f"{data.get('failed_keys', 0)} | "
                    f"{data.get('skipped_records', 0)} | {count_text} |"
                )
            lines.append("")

        render_latency(lines, metrics.get("duration", {}))

    def extract_dingtalk_data(self, grades):
        metrics = grades.get("metrics", {})
        cat_raw = grades.get("category_scores", {})
        cats = []
        for qt in QUESTION_TYPES:
            if qt in cat_raw:
                cat = cat_raw[qt]
                cats.append({
                    "name": qt,
                    "score": cat.get("llm_judge_score", 0),
                    "count": cat.get("total", 0),
                })
        return metrics.get("llm_judge_score", 0), metrics.get("llm_judge_std", 0), cats


if __name__ == "__main__":
    report_main(HaluMemReport())
