...
SCRIPT_NAME="run_halumem_eval.sh"
source "$SCRIPT_DIR/_experiment_utils.sh"

usage() {
    cat <<'EOF'
Usage:
  ./scripts/run_halumem_eval.sh --lib <memory-lib> [options]

Examples:
  ./scripts/run_halumem_eval.sh --lib mem0 --env .env.mem0
  ./scripts/run_halumem_eval.sh --lib memos --env .env --variant long
  ./scripts/run_halumem_eval.sh --lib zep --env .env.zep --version zep_base --workers 2
  ./scripts/run_halumem_eval.sh --lib zep --env .env.zep --to-step 1
  ./scripts/run_halumem_eval.sh --env .env.mem0 --replay results/halumem/mem0-base/
...
Options:
  ...
  --variant <name>        HaluMem dataset variant (medium or long). Default: medium.
  ...
  --skip-failed-streaming <0|1> Streaming mode: explicitly skip failed users. Default: 0.
  ...
  --start-idx <n>         Streaming mode: first user index. Default: 0.
  --end-idx <n>           Streaming mode: last user index. Default: all.
  --restart-unit <0|1>    Streaming mode: delete and discard selected user checkpoints. Default: 0.
...
RESULTS_DIR="$PROJECT_DIR/results/halumem/${LIB}-${VERSION}"
...
if [[ "$STREAMING" == "1" ]]; then
    STREAM_ARGS=(--lib "$LIB" --env "$OMNIMEMEVAL_ENV_FILE" --version "$VERSION" --variant "$VARIANT" --top-k "$TOPK" --allow-empty-search "$ALLOW_EMPTY_SEARCH" --skip-failed-search "$SKIP_FAILED_SEARCH" --start-idx "$START_IDX" --wait-after-ingest "$WAIT_AFTER_INGEST")
    if [[ -n "$END_IDX" ]]; then
    ...
    run_step "Memory Search" \
        python scripts/halumem/hm_search.py \
            --lib "$LIB" \
            --version "$VERSION" \
            --variant "$VARIANT" \
            --top-k "$TOPK" \
...
run_step "Answer Generation" \
    python scripts/halumem/hm_responses.py \
        --lib "$LIB" \
        --version "$VERSION" \
...
run_step "LLM-as-Judge Evaluation" \
    python scripts/halumem/hm_eval.py \
        --lib "$LIB" \
        --version "$VERSION" \
        --llm-workers "$LLM_WORKERS" \
        --num_runs "$NUM_RUNS" \
...
run_step "Metric Calculation" \
    python scripts/halumem/hm_metric.py --lib "$LIB" --version "$VERSION"

run_step "Report Generation" \
    python scripts/halumem/hm_report.py --lib "$LIB" --version "$VERSION" $NOTIFY_FLAG
