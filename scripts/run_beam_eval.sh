...
SCRIPT_NAME="run_beam_eval.sh"
source "$SCRIPT_DIR/_experiment_utils.sh"

usage() {
    cat <<'EOF'
Usage:
  ./scripts/run_beam_eval.sh --lib <memory-lib> [options]

Examples:
  ./scripts/run_beam_eval.sh --lib mem0 --env .env.mem0
  ./scripts/run_beam_eval.sh --lib everos --env .env.everos --scale 10m --streaming 1
  ./scripts/run_beam_eval.sh --lib zep --env .env.zep --version zep_base --workers 2
  ./scripts/run_beam_eval.sh --lib zep --env .env.zep --to-step 1
  ./scripts/run_beam_eval.sh --lib zep --env .env.zep --from-step 2
  ./scripts/run_beam_eval.sh --env .env.mem0 --replay results/beam/mem0-base/

Options:
  --lib <name>            Memory product key, e.g. mem0, zep, supermemory, hindsight.
  ...
  --scale <name>          BEAM evaluation scale (100k/500k/1m/10m/all). Default: 100k.
  --num-runs <n>          Number of LLM-as-a-Judge runs per question. Default: 1.
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            --top-k)
                ...
                ;;
            --scale)
                SCALE="${2:?--scale requires a value}"
                shift 2
                ;;
            --num-runs|--num_runs)
...
RESULTS_DIR="$PROJECT_DIR/results/beam/${LIB}-${VERSION}"
...
if [[ "$STREAMING" == "1" ]]; then
    STREAM_ARGS=(--lib "$LIB" --env "$OMNIMEMEVAL_ENV_FILE" --version "$VERSION" --top-k "$TOPK" --scale "$SCALE" --allow-empty-search "$ALLOW_EMPTY_SEARCH" --start-idx "$START_IDX" --wait-after-ingest "$WAIT_AFTER_INGEST")
    if [[ -n "$END_IDX" ]]; then
    ...
    run_step "Memory Streaming Add/Search/Delete" \
        python scripts/beam/beam_streaming.py "${STREAM_ARGS[@]}"

    run_step "Memory Search" \
        bash -c 'echo "Search results were generated by streaming step."'
else
    run_step "Memory Ingestion" \
        python scripts/beam/beam_ingestion.py --lib "$LIB" --version "$VERSION" --workers "$WORKERS" --scale "$SCALE" $CLEAR_FLAG
    ...
    run_step "Memory Search" \
        python scripts/beam/beam_search.py --lib "$LIB" --version "$VERSION" --top-k "$TOPK" --workers "$WORKERS" --scale "$SCALE" --allow-empty-search "$ALLOW_EMPTY_SEARCH" --skip-failed-search "$SKIP_FAILED_SEARCH"
fi

run_step "Answer Generation" \
    python scripts/beam/beam_responses.py --lib "$LIB" --version "$VERSION" --llm-workers "$LLM_WORKERS" --save-model-input "$SAVE_MODEL_INPUT" --skip-failed-answer "$SKIP_FAILED_ANSWER"

run_step "Nugget Score Evaluation" \
    python scripts/beam/beam_eval.py --lib "$LIB" --version "$VERSION" --llm-workers "$LLM_WORKERS" --num_runs "$NUM_RUNS" --skip-failed-judge "$SKIP_FAILED_JUDGE"

run_step "Metric Calculation" \
    python scripts/beam/beam_metric.py --lib "$LIB" --version "$VERSION"

run_step "Report Generation" \
    python scripts/beam/beam_report.py --lib "$LIB" --version "$VERSION" $NOTIFY_FLAG
